...
## 🔜 Следующий шаг: реализация метода GenerateToken

* Сейчас функция `authService.GenerateToken(role)` ещё не реализована.
* Следующий этап — создать в `auth.go` логику генерации JWT.
* Важно: структура токена должна содержать роль, срок действия и подпись на основе `JWT_SECRET`.
* После этого `dummyLoginHandler` заработает полностью — и можно будет двигаться к настоящей аутентификации.
