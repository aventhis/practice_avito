...
## ✅ Шаг 15. Настройка роутера и запуск HTTP-сервера
- Внутри `apiServer` создала метод `SetupRoutes()`, который настраивает базовые маршруты через `http.ServeMux`.
- В `main.go` вызвала `apiServer.SetupRoutes()` и получила готовый `router`.
- Запустила HTTP-сервер через `http.ListenAndServe`, передав порт из конфига `cfg.Server.Port`.
- Логирую старт сервера через `log.Printf`, используя правильное форматирование для типа `int`.
- Таким образом, сервер стартует автоматически на порту, указанном в `.env`, без захардкоженных значений.
